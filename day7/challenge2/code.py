...
def make_hands(hands: list):
    ...
    for hand in hands:
        ...
        if "J" not in different_cards.keys():
            ...
            if len(different_cards) == 2:
                play_hand = 7
        elif different_cards["J"] == 4:
            play_hand = 7
        else:
            play_hand = 7

        game_hands.append({"cards": hand[0], "0": play_hand, "1": get_value(hand[0][0]), "2": get_value(hand[0][1]), "3": get_value(hand[0][2]), "4":     get_value(hand[0][3]), "5": get_value(hand[0][4]), "bid": hand[1] })
    return game_hands
...
def multiplication(hands: list):
    ...
    for index, hand in enumerate(hands):
        print(hand["cards"], index, hand["bid"], hand["0"], hand["1"], hand["2"], hand["3"], hand["4"], hand["5"])
